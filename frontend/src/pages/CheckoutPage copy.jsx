...
// ── Success Step ───────────────────────────────────────────────────────────
function SuccessStep({ order, onDone }) {
  return (
      ...
      {order && (
        <div style={{ background: "var(--paper)", border: "1px solid var(--border)", borderRadius: "10px", padding: "1rem 1.25rem", marginBottom: "1.75rem", textAlign: "left" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.87rem", marginBottom: "0.4rem" }}>
            ...
            <span style={{ fontWeight: 600, color: "var(--accent)" }}>RM {parseFloat(order.total_amount || 0).toFixed(2)}</span>
          </div>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
        ...
        {cartLoading ? (
          ...
          <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: "14px", padding: "1.75rem" }}>
            ...
            {step === "done" && (
              <SuccessStep order={completedOrder} onDone={onNavigateToOrders} />
            )}
          </div>
        )}
      </div>
