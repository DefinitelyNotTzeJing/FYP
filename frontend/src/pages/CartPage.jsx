...
export default function CartPage({ onNavigateHome, onNavigateToAuth, onNavigateToProfile, onNavigateToWishlist, onNavigateToOrders, onNavigateToCart, onNavigateToReviews }) {
  ...
  const total = items.reduce((sum, i) => {
    const price = parseFloat(i.book?.price || 0);
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "4rem 2rem", color: "var(--muted)" }}>
            ...
            <button
              ...
              style={{ marginTop: "1.5rem", padding: "0.65rem 1.5rem", background: "var(--accent)", color: "white", border: "none", borderRadius: "8px", fontFamily: "var(--font-body)", fontSize: "0.9rem", cursor: "pointer" }}
            >
              Browse Books
            </button>
          </div>
              ...
              {items.map((item) => {
                ...
                return (
                  <div key={item.cart_id || book.book_id} style={{
                    ...
                    {/* Cover */}
                    <div style={{
                      width: 60, height: 80, borderRadius: "4px", overflow: "hidden",
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: "0.92rem", fontWeight: 500, color: "var(--accent)" }}>
                        RM {parseFloat(book.price || 0).toFixed(2)}
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.5rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                        ...
                        <button
                          onClick={() => update(book.book_id, item.quantity + 1)}
                          style={{ width: 28, height: 28, border: "1.5px solid var(--border)", borderRadius: "6px", background: "none", cursor: "pointer", fontSize: "0.9rem" }}
                        >+</button>
                      </div>
                      <button
                        onClick={() => remove(book.book_id)}
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                <button
                  ...
                  style={{ flex: 1, padding: "0.75rem", background: "var(--accent)", color: "white", border: "none", borderRadius: "8px", fontFamily: "var(--font-body)", fontSize: "0.95rem", fontWeight: 500, cursor: "pointer" }}
                >
                  Checkout
                </button>
                <button
                  onClick={clear}
                  style={{ padding: "0.75rem 1rem", background: "none", border: "1.5px solid var(--border)", borderRadius: "8px", fontFamily: "var(--font-body)", fontSize: "0.88rem", cursor: "pointer", color: "var(--muted)" }}
                >
                  Clear
                </button>
              </div>
